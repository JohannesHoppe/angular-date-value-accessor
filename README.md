...
# Angular2: How to use JavaScript Date Object with two-way data bindings (ngModel)
...
It is also possible to fix the default implementation. A selector of `input[type=date]` plus the one of the three FormControl directives would activate the feature magically. But please be aware, that this might break existing implemantions that rely of the old behaviour.